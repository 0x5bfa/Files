...
internal sealed class FtpItemResolver
{
	...
	public async ValueTask<FtpEntryInfo> ResolveAsync(
		FtpPath path,
		CancellationToken cancellationToken = default)
	{
		EnsureWithinRoot(path);

		if (profile.PathComparer.Equals(
			path.Value,
			profile.RootPath.Value))
		{
			return CreateRootEntry();
		}

		var entry = await TryResolveAsync(
			path,
			cancellationToken).ConfigureAwait(false);
		return entry ?? throw new FileNotFoundException(
			"The FTP item could not be resolved.",
			path.Value);
	}

	public async ValueTask<FtpEntryInfo?> TryResolveAsync(
		FtpPath path,
		CancellationToken cancellationToken = default)
	{
		EnsureWithinRoot(path);
		...
		CancellationToken cancellationToken = default)
	{
		...
		var items = await connection
			.ExecuteAsync(
				session => session.GetListingAsync(
					folderPath,
					cancellationToken),
				cancellationToken)
			.ConfigureAwait(false);
	...
	private void EnsureWithinRoot(FtpPath path)
	{
			...
			profile.PathComparer))
		{
			throw new UnauthorizedAccessException(
				$"FTP path '{path.Value}' is outside the configured root.");
		}
	}
}
