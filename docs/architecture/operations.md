...
ストレージ変更は request ベースで、ハンドラーへルーティングします。Files.App は安定した参照から UI 非依存の要求を作り、
...
Files.App は完了後に取得したモデルインスタンスを破棄し、返却参照はフォーカス/表示の意図にだけ使います。表示項目コレクションの正しい情報源は参照セッションです。

## 複数項目のコマンド

ハンドラー契約は意図的に 1 つの要求を処理します。Files.App は複数選択に対して上限付きの順序処理を実行し、進行状況を集約できます。
または、バックエンドが後から専用の一括ハンドラーを追加できます。単一項目の意味を保つことで、すべてのストレージシステムが 1 つの原子的な一括トランザクションを提供するかのような誤解を避けます。

Files.App のバッチアダプターでは次の手順にします。
