...
- Add the native API, COM interface, enum, or struct name to `src/Files.Core/NativeMethods.txt`.
...
3. Build Files.Core or the app project to refresh generated signatures:

   ```powershell
   dotnet build src/Files.Core/Files.Core.csproj -c Debug -p:Platform=x64
   ```
